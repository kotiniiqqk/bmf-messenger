...
async function main() {
    ...
    if (msg.text !== "привет, Боб!")
        fail("текст сообщения не совпал");
    const hist = await jget(`/chats/${chatId}/messages`, tokenB);
    if (hist.status !== 200 || hist.body.messages.length !== 1)
        fail("история сообщений");
